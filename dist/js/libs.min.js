...
* http://
* Copyright (c) 2013 Nicola Tarocco; Licensed MyLicense */
!function(window,undefined){"$:nomunge";var jq_throttle,$=window.jQuery||window.Cowboy||(window.Cowboy={});$.throttle=jq_throttle=function(delay,no_trailing,callback,debounce_mode){function wrapper(){function exec(){last_exec=+new Date,callback.apply(that,args)}function clear(){timeout_id=undefined}var that=this,elapsed=+new Date-last_exec,args=arguments;debounce_mode&&!timeout_id&&exec(),timeout_id&&clearTimeout(timeout_id),debounce_mode===undefined&&elapsed>delay?exec():no_trailing!==!0&&(timeout_id=setTimeout(debounce_mode?clear:exec,debounce_mode===undefined?delay-elapsed:delay))}var timeout_id,last_exec=0;return"boolean"!=typeof no_trailing&&(debounce_mode=callback,callback=no_trailing,no_trailing=undefined),$.guid&&(wrapper.guid=callback.guid=callback.guid||$.guid++),wrapper},$.debounce=function(delay,at_begin,callback){return callback===undefined?jq_throttle(delay,at_begin,!1):jq_throttle(delay,callback,at_begin!==!1)}}(this);
/*! WebLecture Player - v1.0.0 - 2013-10-17
* http://
* Copyright (c) 2013 Nicola Tarocco; Licensed MyLicense */
!function($){function Scrollbar(root,options){function initialize(){return oSelf.update(),setEvents(),oSelf}function setSize(){var sCssSize=sSize.toLowerCase();oContent[options.axis]-oViewport[options.axis],"left"==sDirection?(oThumb.obj.animate({left:iScroll/oScrollbar.ratio},100),oContent.obj.animate({left:-iScroll},100)):(oThumb.obj.css(sDirection,iScroll/oScrollbar.ratio),oContent.obj.css(sDirection,-iScroll)),iMouse.start=oThumb.obj.offset()[sDirection],oScrollbar.obj.css(sCssSize,oTrack[options.axis]),oTrack.obj.css(sCssSize,oTrack[options.axis]),oThumb.obj.css(sCssSize,oThumb[options.axis])}function setEvents(){touchEvents?oViewport.obj[0].ontouchstart=function(event){1===event.touches.length&&(start(event.touches[0]),event.stopPropagation())}:(oThumb.obj.bind("mousedown",start),oTrack.obj.bind("mouseup",drag)),options.scroll&&window.addEventListener?(oWrapper[0].addEventListener("DOMMouseScroll",wheel,!1),oWrapper[0].addEventListener("mousewheel",wheel,!1),oWrapper[0].addEventListener("MozMousePixelScroll",function(event){event.preventDefault()},!1)):options.scroll&&(oWrapper[0].onmousewheel=wheel)}function start(event){$("body").addClass("noSelect");var oThumbDir=parseInt(oThumb.obj.css(sDirection),10);iMouse.start=sAxis?event.pageX:event.pageY,iPosition.start="auto"==oThumbDir?0:oThumbDir,touchEvents?(document.ontouchmove=function(event){event.preventDefault(),drag(event.touches[0])},document.ontouchend=end):($(document).bind("mousemove",drag),$(document).bind("mouseup",end),oThumb.obj.bind("mouseup",end))}function wheel(event){if(oContent.ratio<1){var oEvent=event||window.event,iDelta=oEvent.wheelDelta?oEvent.wheelDelta/120:-oEvent.detail/3;iScroll-=iDelta*options.wheel,iScroll=Math.min(oContent[options.axis]-oViewport[options.axis],Math.max(0,iScroll)),oThumb.obj.css(sDirection,iScroll/oScrollbar.ratio),oContent.obj.css(sDirection,-iScroll),(options.lockscroll||iScroll!==oContent[options.axis]-oViewport[options.axis]&&0!==iScroll)&&(oEvent=$.event.fix(oEvent),oEvent.preventDefault())}}function drag(event){oContent.ratio<1&&(iPosition.now=options.invertscroll&&touchEvents?Math.min(oTrack[options.axis]-oThumb[options.axis],Math.max(0,iPosition.start+(iMouse.start-(sAxis?event.pageX:event.pageY)))):Math.min(oTrack[options.axis]-oThumb[options.axis],Math.max(0,iPosition.start+((sAxis?event.pageX:event.pageY)-iMouse.start))),iScroll=iPosition.now*oScrollbar.ratio,oContent.obj.css(sDirection,-iScroll),oThumb.obj.css(sDirection,iPosition.now))}function end(){$("body").removeClass("noSelect"),$(document).unbind("mousemove",drag),$(document).unbind("mouseup",end),oThumb.obj.unbind("mouseup",end),document.ontouchmove=document.ontouchend=null}var oSelf=this,oWrapper=root,oViewport={obj:$("#thumbnails_container")},oContent={obj:$("#thumbnails")},oScrollbar={obj:$("#scrollbar")},oTrack={obj:$("#track")},oThumb={obj:$("#track-thumb")},sAxis="x"===options.axis,sDirection=sAxis?"left":"top",sSize=sAxis?"Width":"Height",iScroll=0,iPosition={start:0,now:0},iMouse={},touchEvents="ontouchstart"in document.documentElement;return this.update=function(sScroll){oViewport[options.axis]=oViewport.obj[0]["offset"+sSize],oContent[options.axis]=oContent.obj[0]["scroll"+sSize],oContent.ratio=oViewport[options.axis]/oContent[options.axis],oScrollbar.obj.toggleClass("disable",oContent.ratio>=1),oTrack[options.axis]="auto"===options.size?oViewport[options.axis]:options.size,oThumb[options.axis]=Math.min(oTrack[options.axis],Math.max(0,"auto"===options.sizethumb?oTrack[options.axis]*oContent.ratio:options.sizethumb)),oScrollbar.ratio="auto"===options.sizethumb?oContent[options.axis]/oTrack[options.axis]:(oContent[options.axis]-oViewport[options.axis])/(oTrack[options.axis]-oThumb[options.axis]),oScrollbar.ratio<1&&(oScrollbar.ratio=.5),iScroll="relative"===sScroll&&oContent.ratio<=1?Math.min(oContent[options.axis]-oViewport[options.axis],Math.max(0,iScroll)):0,iScroll="bottom"===sScroll&&oContent.ratio<=1?oContent[options.axis]-oViewport[options.axis]:isNaN(parseInt(sScroll,10))?iScroll:parseInt(sScroll,10),setSize()},initialize()}$.tiny=$.tiny||{},$.tiny.scrollbar={options:{axis:"y",wheel:40,scroll:!0,lockscroll:!0,size:"auto",sizethumb:"auto",invertscroll:!1}},$.fn.tinyscrollbar=function(params){var options=$.extend({},$.tiny.scrollbar.options,params);return this.each(function(){$(this).data("tsb",new Scrollbar($(this),options))}),this},$.fn.tinyscrollbar_update=function(sScroll){return $(this).data("tsb").update(sScroll)}}(jQuery);
